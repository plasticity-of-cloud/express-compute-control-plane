    ...
    public void run() {
        try {
            ...
            if (provisioningUrl == null) {
                System.err.println("Error: provisioning URL not configured. Run 'eks-dx configure'.");
                System.exit(1);
            }
            ...
            if ("stopped".equals(state) || "hibernating".equals(state)) {
                System.err.printf("Error: cluster '%s' is stopped. Resume it first: eks-dx resume-cluster %s%n", name, name);
                System.exit(1);
            }
