...
/**
 * Reads/writes ~/.express-compute/config.
 * Endpoint resolution order: ECP_ENDPOINT env → config file → SSM → default.
 */
public class EcpConfig {
    ...
    public String getEndpoint() {
        String env = System.getenv("ECP_ENDPOINT");
        ...
        return "https://express-compute.codriverlabs.ai";
    }
    ...
    public String getStreamUrl() {
        String env = System.getenv("ECP_STREAM_URL");
        if (env != null && !env.isBlank()) return env;
    ...
    public String getProvisioningUrl() {
        String env = System.getenv("ECP_PROVISIONING_URL");
        if (env != null && !env.isBlank()) return env;
