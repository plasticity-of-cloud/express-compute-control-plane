...
class WorkloadIdentityMutatorTest {

    WorkloadIdentityMutator mutator;

    @BeforeEach
    void setUp() {
        mutator = new WorkloadIdentityMutator();
    }
    ...
    void controller_injectsTokenVolume_whenAssociationExists() {
        ...
        assertTrue(result.getSpec().getVolumes().stream()
            .anyMatch(v -> WorkloadIdentityMutator.TOKEN_VOLUME_NAME.equals(v.getName())));
    }
    ...
    void controller_injectsVolumeMount_whenAssociationExists() {
        ...
        assertTrue(result.getSpec().getContainers().get(0).getVolumeMounts().stream()
            .anyMatch(m -> WorkloadIdentityMutator.TOKEN_VOLUME_NAME.equals(m.getName())));
    }
    ...
    void controller_doesNotDuplicateVolume() {
        ...
        long count = result.getSpec().getVolumes().stream()
            .filter(v -> WorkloadIdentityMutator.TOKEN_VOLUME_NAME.equals(v.getName()))
            .count();
        assertEquals(1, count);
    }
        ...
        for (Container container : pod.getSpec().getContainers()) {
            ...
            if (env.stream().noneMatch(e -> "AWS_CONTAINER_CREDENTIALS_FULL_URI".equals(e.getName())))
                env.add(new EnvVarBuilder().withName("AWS_CONTAINER_CREDENTIALS_FULL_URI")
                    .withValue(WorkloadIdentityMutator.CREDENTIALS_URI).build());
            ...
            if (env.stream().noneMatch(e -> "AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE".equals(e.getName())))
                env.add(new EnvVarBuilder().withName("AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE")
                    .withValue(WorkloadIdentityMutator.TOKEN_FILE_PATH).build());

            List<VolumeMount> mounts = container.getVolumeMounts();
            if (mounts == null) { mounts = new ArrayList<>(); container.setVolumeMounts(mounts); }
            if (mounts.stream().noneMatch(m -> WorkloadIdentityMutator.TOKEN_VOLUME_NAME.equals(m.getName())))
                mounts.add(new VolumeMountBuilder().withName(WorkloadIdentityMutator.TOKEN_VOLUME_NAME)
                    .withMountPath(WorkloadIdentityMutator.TOKEN_MOUNT_PATH).withReadOnly(true).build());
        }
    }
    ...
    private void injectTokenVolume(Pod pod) {
        ...
        if (volumes.stream().anyMatch(v -> WorkloadIdentityMutator.TOKEN_VOLUME_NAME.equals(v.getName()))) return;

        volumes.add(new VolumeBuilder()
            .withName(WorkloadIdentityMutator.TOKEN_VOLUME_NAME)
            .withNewProjected()
                .withSources(new VolumeProjectionBuilder()
                    .withNewServiceAccountToken()
                        .withAudience(WorkloadIdentityMutator.TOKEN_AUDIENCE)
