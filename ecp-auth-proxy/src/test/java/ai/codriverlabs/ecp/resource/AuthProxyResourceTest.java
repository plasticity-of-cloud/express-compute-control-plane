...
class AuthProxyResourceTest {
    ...
    @Mock CredentialServiceClient forwardingService;

    AuthProxyResource resource;

    @BeforeEach
    void setUp() {
        resource = new AuthProxyResource();
    ...
    void assumeRole_returns200_whenLambdaSucceeds() {
        ...
        when(forwardingService.forward(eq("cluster"), any()))
            .thenReturn(new CredentialServiceClient.ForwardResult(200, "{\"credentials\":{}}"));

        var req = new AuthProxyResource.AgentRequest();
    ...
    void assumeRole_forwardsToLambda_afterTokenReview() {
        when(tokenValidationService.validateToken("valid-token", "cluster"))
            .thenReturn(new TokenValidationService.TokenClaims(
                "default", "my-sa", null, null, null, "system:serviceaccount:default:my-sa", null));
        when(forwardingService.forward(eq("cluster"), any()))
            .thenReturn(new CredentialServiceClient.ForwardResult(200, "{}"));
    ...
    void assumeRole_propagatesLambdaErrorStatus() {
        when(tokenValidationService.validateToken("valid-token", "cluster"))
            .thenReturn(new TokenValidationService.TokenClaims(
                "default", "my-sa", null, null, null, "system:serviceaccount:default:my-sa", null));
        when(forwardingService.forward(eq("cluster"), any()))
            .thenReturn(new CredentialServiceClient.ForwardResult(404,
